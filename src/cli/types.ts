...
export type Config = {
	...
	clientIntegration?: {
		 ...
		 * the output.
		 *
		 * If set to `true`, Prismic clients will automatically be typed with the generated Custom Types
		 * and Slices.
		 *
		 * **Note**: If your project queries content from multiple Prismic repositories, set
		 * `includeCreateClientInterface` to `true` for the primary repository and `false` or any other
		 * repository. The generated `AllDocumentTypes` type for non-primary repositories can be
		 ...
		 *
		 * If set to `true`, a `Content` namespace from `@prismicio/client` will be available to import
		 * to easily access types for your Prismic repository content.
		 *
		 * **Note**: If your project queries content from multiple Prismic repositories, set
		 * `includeContentNamespace` to `true` for the primary repository and `false` or any other
		 * repository. Types for non-primary repositories should be imported directly from the generated
		 * file rather than via the `Content` namespace.
		 *
		 * @defaultValue `true`
		 */
	...
	fields?: {
		...
		embed?: {
			/**
			 * An object mapping oEmbed providers to their type. The type should be provided as a string
		...
		integrationFields?: {
			/**
			 * An object mapping catalog IDs to their type. The type should be provided as a string and is
			 * added directly into the generated types.
			 *
			 * @example
			 * 	```ts
			 * 	{
